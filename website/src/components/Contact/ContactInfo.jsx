...
const ContactInfo = () => {
  ...
  // Measure one real card + gap so the reference-style cards
  // can auto-scroll continuously without changing their size.
  useEffect(() => {
    const updateCarousel = () => {
      const viewport = cardsViewportRef.current;
      const track = cardsTrackRef.current;

      if (!viewport || !track || window.innerWidth < 1024) return;
      ...
      const step = secondRect.top - firstRect.top;
      const gap = Math.max(step - cardHeight, 0);

      setCardsCarouselStyle({
        "--contact-step": `${step}px`,
        height: `${cardHeight * 3 + gap * 2}px`,
      });
    };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 min-[1920px]:gap-10 min-[3840px]:gap-16 items-stretch">
          ...
          >
            ...
            >
              ...
              <div className="flex flex-col gap-5 sm:gap-6 min-[1920px]:gap-8 min-[3840px]:gap-12 w-full">
                {/* ----- WhatsApp ----- */}
                <div
                  aria-hidden="true"
                  className="
